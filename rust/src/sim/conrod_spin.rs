    ...
    pub fn new() -> SimSpin {
        ...
        let (shapes, vertex_buffers) = unsafe {
            ...
            for (shape, buffer) in shapes.iter_mut().zip(vertex_buffers.iter_mut()) {
                // // note: this requires phi to start at pi/2
                // let y1 = R0 + (i as f32)*(LED + SPACE);
                // let y2 = y1 + LED;
                // let new_shape = vec![
                //     Vertex::new(0.0, y1, Rgb::new(0.0, 0.0, 0.0)),
                //     Vertex::new(0.0, y2, Rgb::new(0.0, 0.0, 0.0))
                //         ];
